...
impl ProjectParser {
    ...
    fn parse_cargo_project(&self, cargo_path: &Path) -> Result<(ProjectInfo, Vec<ProjectDependency>)> {
        ...
        if let Some(dev_deps) = cargo_toml.dev_dependencies {
            for (name, _spec) in dev_deps {
                dependencies.push(self.create_dependency(name, None, Some("dev".to_string()), ProjectLanguage::Rust));
            }
        }
    ...
    fn parse_maven_project(&self, pom_path: &Path) -> Result<(ProjectInfo, Vec<ProjectDependency>)> {
        ...
        let project_name = artifact_id_re.find(&content)
            .map(|m| content[m.range()].replace("<artifactId>", "").replace("</artifactId>", ""))
            .unwrap_or_else(|| "unknown-java-project".to_string());

        let project_version = version_re.find(&content)
            .map(|m| content[m.range()].replace("<version>", "").replace("</version>", ""));
    ...
    fn parse_go_project(&self, go_mod_path: &Path) -> Result<(ProjectInfo, Vec<ProjectDependency>)> {
        ...
        let module_name = module_re.captures(&content)
            .and_then(|caps| caps.get(1))
            .map(|m| m.as_str().to_string())
            .unwrap_or_else(|| "unknown-go-project".to_string());

        let go_version = go_version_re.captures(&content)
            .and_then(|caps| caps.get(1))
            .map(|m| m.as_str().to_string());

        let project_info = ProjectInfo {
            name: module_name.split('/').last().unwrap_or(&module_name).to_string(),
            version: go_version,
    ...
    fn parse_requirements_project(&self, req_path: &Path, scan_path: &Path) -> Result<(ProjectInfo, Vec<ProjectDependency>)> {
        ...
        for line in content.lines() {
            ...
            if let Some(caps) = requirement_re.captures(line) {
                let name = caps.get(1).map(|m| m.as_str().to_string()).unwrap_or_default();
                let version = caps.get(2).map(|m| m.as_str().to_string());
                dependencies.push(self.create_dependency(name, version, None, ProjectLanguage::Python));
            }
        }
    ...
    fn parse_npm_project(&self, package_path: &Path) -> Result<(ProjectInfo, Vec<ProjectDependency>)> {
        ...
        let project_name = package_json["name"].as_str().unwrap_or("unknown-js-project").to_string();
        ...
        if let Some(deps) = package_json["dependencies"].as_object() {
            for (name, version) in deps {
                ...
                dependencies.push(self.create_dependency(name.clone(), version_str, None, ProjectLanguage::JavaScript));
            }
        }
        ...
        if let Some(dev_deps) = package_json["devDependencies"].as_object() {
            for (name, version) in dev_deps {
                let version_str = version.as_str().map(|s| s.to_string());
                dependencies.push(self.create_dependency(name.clone(), version_str, Some("dev".to_string()), ProjectLanguage::JavaScript));
            }
        }
    ...
    fn parse_makefile_project(&self, makefile_path: &Path, scan_path: &Path) -> Result<(ProjectInfo, Vec<ProjectDependency>)> {
        let content = fs::read_to_string(makefile_path)
            .context("Failed to read Makefile")?;
        ...
        let language = if content.contains("g++") || content.contains("clang++") || content.contains(".cpp") || content.contains(".cxx") {
            ProjectLanguage::Cpp
        } else {
            ProjectLanguage::C
        };
    ...
    fn parse_gradle_project(&self, _gradle_path: &Path) -> Result<(ProjectInfo, Vec<ProjectDependency>)> {
    ...
    fn parse_pipfile_project(&self, _pipfile_path: &Path) -> Result<(ProjectInfo, Vec<ProjectDependency>)> {
    ...
    fn parse_gemfile_project(&self, _gemfile_path: &Path) -> Result<(ProjectInfo, Vec<ProjectDependency>)> {
    ...
    fn parse_composer_project(&self, _composer_path: &Path) -> Result<(ProjectInfo, Vec<ProjectDependency>)> {
    ...
    fn parse_cmake_project(&self, _cmake_path: &Path, scan_path: &Path) -> Result<(ProjectInfo, Vec<ProjectDependency>)> {
        let project_name = scan_path.file_name()
            .and_then(|n| n.to_str())
    ...
    fn parse_podspec_project(&self, _podspec_path: &Path) -> Result<(ProjectInfo, Vec<ProjectDependency>)> {
    ...
    pub fn get_crypto_package_info(&self, package_name: &str, language: &ProjectLanguage) -> Option<&CryptoPackageInfo> {
    ...
    fn populate_crypto_packages(&mut self) {
        ...
        python_packages.insert("pycryptodome".to_string(), CryptoPackageInfo {
            algorithms: vec!["RSA".to_string(), "AES".to_string()],
            is_pqc_vulnerable: true,
            description: "PyCryptodome".to_string(),
        });
        self.crypto_packages.insert(ProjectLanguage::Python, python_packages);

        // JavaScript packages
        let mut js_packages = HashMap::new();
        js_packages.insert("crypto-js".to_string(), CryptoPackageInfo {
            algorithms: vec!["AES".to_string(), "SHA-256".to_string()],
            is_pqc_vulnerable: false,
            description: "JavaScript crypto library".to_string(),
        });
        self.crypto_packages.insert(ProjectLanguage::JavaScript, js_packages);

        // C/C++ libraries (detected from Makefiles)
        let mut c_packages = HashMap::new();
        c_packages.insert("ssl".to_string(), CryptoPackageInfo {
            algorithms: vec!["RSA".to_string(), "ECDSA".to_string(), "AES".to_string()],
            is_pqc_vulnerable: true,
            description: "OpenSSL".to_string(),
        });
        c_packages.insert("crypto".to_string(), CryptoPackageInfo {
            algorithms: vec!["RSA".to_string(), "ECDSA".to_string(), "AES".to_string()],
            is_pqc_vulnerable: true,
            description: "OpenSSL Crypto".to_string(),
        });
        c_packages.insert("sodium".to_string(), CryptoPackageInfo {
        ...
        self.crypto_packages.insert(ProjectLanguage::C, c_packages.clone());
        self.crypto_packages.insert(ProjectLanguage::Cpp, c_packages);

        // Go packages
        let mut go_packages = HashMap::new();
        go_packages.insert("golang.org/x/crypto".to_string(), CryptoPackageInfo {
            algorithms: vec!["RSA".to_string(), "ECDSA".to_string(), "AES".to_string()],
            is_pqc_vulnerable: true,
            description: "Go extended crypto".to_string(),
        });
        self.crypto_packages.insert(ProjectLanguage::Go, go_packages);
    }
}
...
"#;
        ...
        let crypto_deps: Vec<_> = dependencies.iter().filter(|d| d.is_crypto_related).collect();
