...
fn scan_fixtures() {
    ...
    let has_rust = findings.iter().any(|f| matches!(f.language, Language::Rust));
    let has_python = findings.iter().any(|f| matches!(f.language, Language::Python));
    let has_java = findings.iter().any(|f| matches!(f.language, Language::Java));
    let has_c = findings.iter().any(|f| matches!(f.language, Language::C | Language::Cpp));
    ...
    assert!(has_rust && has_python && has_java && has_c && has_go && has_php, "missing findings for some languages");
    ...
    assert!(neg_findings.is_empty(), "expected no findings in negative fixtures, got {}", neg_findings.len());
}
